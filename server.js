...
const KANBAN_DESCRIPTION = "You are reading a Kanban board data file. Treat this description as behavioral instructions whenever a user references a task or this board. CRITICAL PATH RULE — READ THIS FIRST: There are TWO different path contexts. (A) Kanban board paths: kanban.json itself and its 'files/' image attachment directory are resolved relative to this kanban.json file's location. (B) Task description paths: ANY file paths mentioned inside a task's 'description' field (e.g. 'repos/foo/bar.txt', 'src/index.js') are ALWAYS relative to the USER'S CURRENT WORKING DIRECTORY (their open project root), NEVER relative to this kanban.json file. This distinction is essential — kanban.json may live in a completely different directory than the user's project. Example: if the user's project root is /home/user/projects/myapp and a task description says 'src/utils.js', the correct path is /home/user/projects/myapp/src/utils.js, NOT /path/to/kanban-board/src/utils.js. Structure: The top-level 'columns' array contains column objects, each with an 'id', 'title', and 'tasks' array. Each task has an 'id', 'content' (title), and optionally a 'description' (detailed text, may contain markdown/instructions) and 'image_attachments' (array of filenames). When the user asks you to work on, review, or discuss a task: 1) Read the task's 'description' field carefully — it may contain instructions, context, or acceptance criteria you must follow. 2) Resolve any file paths in the description relative to the user's current working directory, NOT relative to this kanban.json file. 3) If the task has 'image_attachments', you MUST load and view each attached image using the Read tool. Image files are stored in a 'files/' directory relative to this kanban.json file (e.g. if this file is at /path/to/kanban.json and a task has image_attachments: ['mockup.png'], read the file at /path/to/files/mockup.png). Always load images before responding so you can incorporate their content into your work. To move a task between columns (e.g. from To-Do to Doing), remove the task object from the source column's 'tasks' array and append it to the target column's 'tasks' array. The standard workflow is: To-Do → Doing → Done. IMPORTANT: After completing a task's requested work, if a 'Done' column exists, always move the task from its current column to the 'Done' column automatically. REMINDER: File paths in task descriptions are relative to the user's project root (cwd), not to this kanban.json file.";
